...
<body class="bg-slate-100 text-slate-800 antialiased">
    ...
    <?php 
        ...
        $admin_role = strtolower($this->session->userdata('role'));
    ?>
    ...
    <?php if ($admin_role === 'superadmin' || $this->session->userdata('username') === 'superadmin'): ?>
    ...
    <a href="<?= base_url('admin/kelola_bendungan') ?>" class="menu-item flex items-center gap-3 px-4 py-3 rounded-xl text-sm <?= ($current_url == 'admin/kelola_bendungan') ? 'menu-active' : 'text-slate-300 font-medium' ?>">
        <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z"/></svg>
        <span>Kelola Bendungan</span>
    </a>

    <a href="<?= base_url('admin/kelola_manual') ?>" class="menu-item flex items-center gap-3 px-4 py-3 rounded-xl text-sm <?= ($current_url == 'admin/kelola_manual') ? 'menu-active' : 'text-slate-300 font-medium' ?>">
        <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z"/></svg>
        ...
        <div class="p-4 border-t border-white/10">
            <div class="flex items-center gap-3 bg-white/10 rounded-xl px-3 py-2.5 mb-3">
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-[10px] text-blue-300 mt-0.5 text-capitalize"><?= $this->session->userdata('role') ?></p>
                </div>
            </div>
