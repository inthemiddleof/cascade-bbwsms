...
<body class="bg-slate-50 text-slate-800 antialiased">
    ...
    <nav id="mobile-nav" class="lg:hidden">
        ...
        <div id="mobile-menu" class="hidden mt-2 mx-1 bg-white/95 backdrop-blur-xl rounded-2xl border border-white/40 shadow-xl overflow-hidden">
            <div class="px-4 py-5 space-y-1">
                ...
                <?php if ($is_superadmin): ?>
                <!-- Superadmin Infrastructure Menus -->
                <div class="border-t border-slate-200 my-3"></div>
                <p class="px-3 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Infrastruktur</p>
    ...
    <aside id="sidebar" class="hidden lg:flex fixed top-0 left-0 z-50 h-full w-72 bg-white border-r border-slate-200 flex-col shadow-sm">
        ...
        <nav id="sidebar-nav" class="flex-1 overflow-y-auto py-3 px-3 space-y-0.5">
            <p class="px-3 mb-2 text-[9px] font-bold text-slate-400 uppercase tracking-[0.15em]">Menu Utama</p>
            
            <!-- Dashboard -->
            <a href="<?= base_url($base_url) ?>" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all <?= ($current_url == $base_url || $current_url == $base_url . '/index') ? 'bg-darkblue text-white font-bold shadow-sm' : 'text-slate-600 hover:bg-slate-50 hover:text-darkblue' ?>">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zm0 9.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25zM13.5 6a2.25 2.25 0 012.25-2.25H18A2.25 2.25 0 0120.25 6v2.25A2.25 2.25 0 0118 10.5h-2.25a2.25 2.25 0 01-2.25-2.25V6zm0 9.75a2.25 2.25 0 012.25-2.25H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-2.25A2.25 2.25 0 0113.5 18v-2.25z"/></svg>
                <span>Dashboard</span>
            </a>
            ...
            <?php if ($is_superadmin): ?>
            <a href="<?= base_url('superadmin/kelola_admin') ?>" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all <?= (strpos($current_url, 'superadmin/kelola_admin') !== false) ? 'bg-darkblue text-white font-bold shadow-sm' : 'text-slate-600 hover:bg-slate-50 hover:text-darkblue' ?>">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                <span>Kelola Admin</span>
            </a>
            <a href="<?= base_url('superadmin/kelola_pos') ?>" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all <?= (strpos($current_url, 'superadmin/kelola_pos') !== false) ? 'bg-darkblue text-white font-bold shadow-sm' : 'text-slate-600 hover:bg-slate-50 hover:text-darkblue' ?>">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z"/></svg>
                <span>Kelola Pos</span>
            </a>
            <?php else: ?>
            <a href="<?= base_url('admin/kelola_petugas') ?>" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all <?= (strpos($current_url, 'admin/kelola_petugas') !== false) ? 'bg-darkblue text-white font-bold shadow-sm' : 'text-slate-600 hover:bg-slate-50 hover:text-darkblue' ?>">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z"/></svg>
            ...
            <?php $manual_url = $is_superadmin ? 'superadmin/kelola_manual' : 'admin/kelola_manual'; ?>
            <a href="<?= base_url($manual_url) ?>" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all <?= (strpos($current_url, $manual_url) !== false) ? 'bg-darkblue text-white font-bold shadow-sm' : 'text-slate-600 hover:bg-slate-50 hover:text-darkblue' ?>">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
            ...
            <div class="mt-4 mb-2 px-3">
                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-[0.15em]">Infrastruktur</p>
            </div>

            <a href="<?= base_url('superadmin/kelola_embung') ?>" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all <?= (strpos($current_url, 'superadmin/kelola_embung') !== false) ? 'bg-darkblue text-white font-bold shadow-sm' : 'text-slate-600 hover:bg-slate-50 hover:text-darkblue' ?>">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"/></svg>
                <span>Kelola Embung</span>
            </a>

            <a href="<?= base_url('superadmin/kelola_pengaman_pantai') ?>" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all <?= (strpos($current_url, 'superadmin/kelola_pengaman_pantai') !== false) ? 'bg-darkblue text-white font-bold shadow-sm' : 'text-slate-600 hover:bg-slate-50 hover:text-darkblue' ?>">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                <span>Pengaman Pantai</span>
            </a>

            <a href="<?= base_url('superadmin/kelola_pengendali_sedimen') ?>" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all <?= (strpos($current_url, 'superadmin/kelola_pengendali_sedimen') !== false) ? 'bg-darkblue text-white font-bold shadow-sm' : 'text-slate-600 hover:bg-slate-50 hover:text-darkblue' ?>">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/></svg>
                <span>Pengendali Sedimen</span>
            </a>

            <a href="<?= base_url('superadmin/kelola_irigasi') ?>" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all <?= (strpos($current_url, 'superadmin/kelola_irigasi') !== false) ? 'bg-darkblue text-white font-bold shadow-sm' : 'text-slate-600 hover:bg-slate-50 hover:text-darkblue' ?>">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 10h18M3 14h18m-9-4v8m-7 0h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
            ...
            <!-- Export Import -->
            <div class="mt-4 mb-2 px-3">
                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-[0.15em]">Tools</p>
            </div>
        ...
        <!-- User Profile & Logout -->
        <div class="flex-shrink-0 border-t border-slate-200 px-4 py-3">
